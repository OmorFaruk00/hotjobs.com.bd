                  ...
                  <div class="table-responsive">
                    <table class="table table-bordered">
                      ...
                      <tr v-for="(row,index) in fetchTableData" :key="index">
                        <td>
                          <a :href="`/${row.job_details.id}/${row.job_details.employer.slug}/${row.job_details.slug}`" target="_blank">
                            {{ row.job_details.job_title }}
                          </a>

                          <br>

                          <span style="font-size: .8rem">{{ row.job_details.employer.company_name }}</span> <br>
...
export default {
  ...
  methods: {
    async getApplyJobPostDetails() {

      var token = window.$nuxt.$cookies.get('token');

      return await this.$axios.get('employee/report/employee-apply-job-post-report/'+ '?token=' + token)
        .then((response) => {
          this.apply_job_post = response.data;
        })
        .catch((error) => {
  ...
  created() {
    this.getApplyJobPostDetails();
  },

  computed: {
    itemLength() {
      return this.apply_job_post.length
    },

    fetchTableData() {
      return this.apply_job_post.filter((data) => data.job_details.job_title.toLowerCase().match(this.job_title.toLowerCase()));
    },
  },
