                  ...
                  <div class="table-responsive">
                    <table class="table table-bordered">
                      ...
                      <tr v-for="(row,index) in fetchTableData" :key="index">
                        <td>
                          <a :href="`/t/${row.tender_job_details.id}/${row.tender_job_details.employer.slug}/${row.tender_job_details.slug}`" target="_blank">
                            {{ row.tender_job_details.title }}
                          </a>

                          <br>

                          <span style="font-size: .8rem">{{ row.tender_job_details.employer.company_name }}</span> <br>
...
export default {
  ...
  methods: {
    async getApplyTenderJobDetails() {

      var token = window.$nuxt.$cookies.get('token');

      return await this.$axios.get('employee/report/employee-apply-tender-job-report/'+ '?token=' + token)
        .then((response) => {
          this.apply_tender_job = response.data;
        })
        .catch((error) => {
  ...
  created() {
    this.getApplyTenderJobDetails();
  },

  computed: {
    itemLength() {
      return this.apply_tender_job.length
    },

    fetchTableData() {
      return this.apply_tender_job.filter((data) => data.tender_job_details.title.toLowerCase().match(this.job_title.toLowerCase()));
    },
  },
